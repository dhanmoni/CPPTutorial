...
auto itr = min_element(vec.begin(), vec.end());  // 7 
// Generalized form: min_element()

auto pair_of_itr = minmax_element(vec.begin(), vec.end(),  // {60, 69} 
...
auto itr = find_if(vec.begin(), vec.end(), [](int x){ return x>80; });

auto itr = find_if_not(vec.begin(), vec.end(), [](int x){ return x>80; });

auto itr = search_n(vec.begin(), vec.end(), 2, 69);  // Consecutive 2 items of 69
